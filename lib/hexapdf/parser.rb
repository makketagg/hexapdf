...
module HexaPDF
  ...
  class Parser
    ...
    # Tries to reconstruct the PDF document by serially reading the objects of the file and
    # returning a Revision object with them.
    #
    # If the file contains multiple cross reference sections, all objects will just be put into a
    # single revision, later objects overwriting prior ones.
    def reconstruct_revision
      ...
      if trailer.empty?
        raise_malformed("Could reconstruct malformed PDF because no trailer was found", pos: 0)
      end
